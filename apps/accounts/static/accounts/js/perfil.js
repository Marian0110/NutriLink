document.addEventListener('DOMContentLoaded', async () => {
  ...
  function getSessionData() {
    ...
    // 🔒 Refuerza el valor en sessionStorage para uso posterior (ej. formulario)
    if (correo) {
    ...
    if (response.ok) {
      ...
      if (data && data.primer_nombre) {
        ...
        if (data.foto_base64) {
          ...
          profileImg.src = data.foto_base64;
...
profileForm.addEventListener("submit", async function (e) {
  ...
  console.log("Correo desde sessionStorage:", correo);

  const formData = new FormData();
  formData.append("correo", correo);
  formData.append("primer_nombre", document.getElementById("pNombre").value || '');
  formData.append("segundo_nombre", document.getElementById("sNombre").value || '');
  formData.append("apellido_paterno", document.getElementById("apePater").value || '');
  formData.append("apellido_materno", document.getElementById("apeMater").value || '');
  ...
  const archivoImagen = document.getElementById("imagenPerfil").files[0];
  if (archivoImagen) {
    console.log("Archivo de imagen seleccionado:", archivoImagen.name, "-", archivoImagen.size, "bytes");
    formData.append("foto", archivoImagen);
  } else {
    console.log("No se seleccionó imagen de perfil.");
  }
